  ...
  if (interaction.isButton()) {
    ...
    switch (interaction.customId) {
      ...
      case "stop":
        ...
        queues.delete(guildId);
        await interaction.editReply("⏹ Остановлено");
        break;
    }
    return;
  }
  ...
  if (commandName === "play") {
    ...
    if (!player) {
      try {
        ...
        return interaction.editReply("❌ Lavalink не отвечает (попробуй другой узел)");
      }
    }
    let result;
    try {
      ...
      return interaction.editReply("❌ Ошибка поиска");
    }
...
shoukaku.on("trackEnd", (player, track, reason) => {
  ...
  playNext(player.connection.guildId);
});
